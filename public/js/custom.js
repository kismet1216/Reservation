...
angular.module("reservations", []).controller("assignmentController", function( $scope, get, set){
 $scope.count=6;
 $scope.countrow=7;
 $scope.countfour=4;
 $scope.countseat=0;
   ...
   $(".seats ul li a").on("click", function(){
     ...
     if($(this).hasClass("booked")){
		...
		$scope.selectedClass=$scope.selectedClass-1;
	       ...
	       $scope.selectedClass=get._howMany($scope.selectedClass, '1');
		   $scope.check=$scope.selectedClass;
			if(++$scope.check=='1'){$(".btn-success").removeAttr('disabled');} 
			 if($scope.selectedClass< '1'){
...
angular.module("reservations", []).service("set", function(){
...
angular.module("reservations", []).factory("get", function(){
  var fact={};
